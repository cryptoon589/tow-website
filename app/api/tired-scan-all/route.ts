...
export async function POST(request: NextRequest) {
  try {
    ...
    const maxWallets = Math.min(
      100,
      Math.max(1, Number(body.maxWallets ?? 25))
    );
    ...
    const transactionsScanned =
      results.reduce(
        (sum, item) =>
          sum + Number(item.scanned ?? 0),
        0
      );
    ...
    return NextResponse.json(
      {
        error:
          "Could not scan registered wallets.",
        details:
